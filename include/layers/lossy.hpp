...
std::vector<T> apply_lossy_preprocessing(const T* src, const T* ft, size_t n, float lossy_rate, bool is_bf16 = false) { // [UPDATED] Added is_bf16
    ...
    // 核心性能修复：将分支判断移出循环，消除80亿次分支预测开销，允许编译器SIMD向量化
    if constexpr (sizeof(T) == 4) {
        for(size_t i = 0; i < n; ++i) {
            double diff = std::abs(f_ft_ptr[i] - f_src_ptr[i]);
        ...
        if (is_bf16) {
            for(size_t i = 0; i < n; ++i) {
                float v_src = bf16_to_float(src[i]);
                float v_ft  = bf16_to_float(ft[i]);
                double diff = std::abs(v_ft - v_src);
                if (diff < threshold_val) {
                    processed_ft[i] = src[i];
                } else {
                    processed_ft[i] = ft[i] & mask;
            ...
            for(size_t i = 0; i < n; ++i) {
                float v_src = fp16_to_float(src[i]);
                float v_ft  = fp16_to_float(ft[i]);
                double diff = std::abs(v_ft - v_src);
                if (diff < threshold_val) {
                    processed_ft[i] = src[i];
                } else {
